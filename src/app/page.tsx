...
export default function Home() {
  return <>
    ...
    <section className="section compact white"><div className="container"><SectionHead eyebrow="从真实目标出发" title="我想用 AI 做什么？" action="查看全部路径" href="/learn"/><div className="goal-grid">{goals.map(({title, detail, icon:Icon})=><Link className="goal-card" href={`/learn?goal=${title}`} key={title}><span className="goal-icon"><Icon size={22}/></span><h3>{title}</h3><p>{detail}</p></Link>)}</div></div></section>
